...
// helper to convert fastgltf filter to vulkan mipmap mode
...
std::optional<std::shared_ptr<LoadedGLTF>> loadGltf(VulkanEngine* engine, std::string_view filePath)
{
	...
	// estimate the descriptors we will need accurately
	...
	// load all meshes
	// using same vectors for all meshes so that memory doesnt reallocate as often
	...
	for (fastgltf::Mesh& mesh : gltf.meshes) {
		...
		// dont want to merge them by error
		indices.clear();
		vertices.clear();
		...
		for (auto&& p : mesh.primitives) {
			...
			// load vertices
			{
				...
				fastgltf::iterateAccessorWithIndex<glm::vec3>(gltf, posAccessor, [&](glm::vec3 v, size_t idx) {
					...
					newVtx.uv_x = 0;
					newVtx.uv_y = 0;
					vertices[initialVertex + idx] = newVtx;
					});
			}
			...
			if (uv != p.attributes.end()) {
				fastgltf::iterateAccessorWithIndex<glm::vec2>(gltf, gltf.accessors[(*uv).second], [&](glm::vec2 v, size_t idx) {
					vertices[initialVertex + idx].uv_x = v.x;
					vertices[initialVertex + idx].uv_y = v.y;
					});
			}
	...
	for (fastgltf::Node& node : gltf.nodes) {
		...
		if (node.meshIndex.has_value()) { // has mesh, hook it to the mesh pointer and allocate it with meshnode class
			newNode = std::make_shared<MeshNode>();
			static_cast<MeshNode*>(newNode.get())->mesh = meshes[*node.meshIndex];
		}
		else { // has no mesh, create a normal node
